...
| `OVER_PRESSURE` | 压力过高 | 实测压力高于参考值允许范围 | `WARNING` → `FAULT` | 降额 → 停止 | 关注设备压力，必要时停机 | 检查压力传感器、目标压力、容差、液压回路阻塞和阀动作是否匹配。**WARNING持续2秒后自动升级为FAULT并停机** |
| `UNDER_PRESSURE` | 压力偏低 | 实测压力低于参考值允许范围 | `WARNING` | 提示 | 检查是否存在泄压、供压不足或工艺条件未满足 | 检查泵输出、泄漏、保压设定、容差设置、反馈采样质量 |
| `FLOW_DEVIATION` | 流量偏差过大 | 实测流量与参考流量偏差过大 | `WARNING` → `FAULT` | 降额 → 停止 | 检查执行机构响应和油路状态 | 检查泵速换算、流量反馈、执行机构负载、阀开度和流量容差。**WARNING持续3秒后自动升级为FAULT并停机** |
| `POSITION_DEVIATION` | 位置偏差过大 | 实测位置与目标位置偏差过大 | `WARNING` → `FAULT` | 提示 → 停止 | 检查机构是否未到位或位置反馈异常 | 检查编码器 / 位移传感器、目标位置、位置容差、机械卡阻。**WARNING持续2秒后自动升级为FAULT并停机** |
| `VELOCITY_DEVIATION` | 速度偏差过大 | 实测速度与参考速度偏差过大 | `WARNING` → `FAULT` | 提示 → 停止 | 检查速度响应是否异常 | 检查速度反馈计算、加减速参数、负载突变和容差设定。**WARNING持续2秒后自动升级为FAULT并停机** |
...
| `PUMP_DIRECTION_CONFLICT` | 泵方向冲突 | 单泵下多轴方向互斥（EXTEND + RETRACT 同时 active） | `WARNING` | 无 | PLC 应自行决定降级或中止当前动作 | 检查当前 active 轴的方向规划是否互斥，是否需分时复用泵源 |
...
控制器支持 `WARNING` 级别自动升级为 `FAULT` 级别的机制，用于处理持续存在的偏差：
...
2. **升级条件**：
   - `WARNING` 持续存在时间超过配置的升级时间
   - 升级期间误差仍未恢复
   - 启用升级功能（`enableFaultEscalation = true`）
...
| 阶段 | 严重级别 | 颜色 | 动作 | 显示建议 |
| --- | --- | --- | --- | --- |
| 初始触发 | `WARNING` | 黄色 | 提示/降额 | "压力过高 - 已降额运行" |
| 升级中 | `WARNING` | 黄色闪烁 | 提示/降额 | "压力过高 - 即将停机（X秒后）" |
| 已升级 | `FAULT` | 红色 | 停止 | "压力过高 - 已停机（保护）" |
...
1. **启动阶段抑制**（默认500ms）：段启动后短时间内不触发诊断
2. **切段阶段抑制**（默认300ms）：段切换后短时间内不触发诊断
3. **闭环建立抑制**（默认200ms）：压力闭环建立期间降低诊断敏感度
4. **去抖动逻辑**（默认100-200ms）：误差需要持续一段时间才触发

这些抑制机制会减少误报，确保升级机制在真正需要时才触发。
