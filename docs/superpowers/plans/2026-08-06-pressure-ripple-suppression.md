...
**Current status:** Tasks 0-5 are implemented on `feature/pressure-ripple-suppression`
and pass the host suite. Tasks 6-9 remain mandatory gates; the checked-in ripple table
is an uncalibrated zero-entry stub and must not be enabled or merged to `master`.
...
unbounded/nonfinite target, target RPM outside `[min_rpm, max_rpm]`, or absolute load flow
above `1.666667e-3 m3/s`. Only an irregular `dt_s` is normalized to one safe 1 ms substep;
physical target RPM is rejected rather than clamped. Motor natural frequency `<= 50 Hz`,
damping `<= 2`, and acceleration limit `<= 100000 RPM/s`. The
...
Create tests for the current `HYD_RippleComp_Scan()` contract. The first valid sample
seeds adjacent-sample state; a second modulo-360 sample must produce finite output only
when the calibrated table, timestamp, RPM direction, and angle increment are valid.
...
Use these C99 types. Amplitudes are absolute RPM-domain values; the table sign is part of
the calibration result. The generated table is included only by the compensator
implementation so every translation unit does not carry a private copy of the table:
...
The caller or HAL adapter owns this state per axis and passes it to
`HYD_MotionControlFB_ScanWithPumpFeedback()`; `HYD_MotionControlFB` does not store it.
The state stores only adjacent-sample validation information, never a table pointer,
configuration, packet copy, or diagnostic snapshot. The public angle remains modulo-360
degrees. The implementation folds angle deltas into [-180, 180), rejects increments
above the physical RPM-derived limit, and treats zero delta as valid at zero RPM. Use a
bounded lookup table for sine/cosine and no adaptive coefficient state.
...
```c
float delta_rpm = amp13_rpm * lookup_sin(13.0f * angle_rad + phase13_rad) +
                  amp26_rpm * lookup_sin(26.0f * angle_rad + phase26_rad);
delta_rpm = clampf(-0.30f * pumpSpeedLimit,
                  delta_rpm,
                  0.30f * pumpSpeedLimit);
```

Use the table's measured sign; do not prepend a generic negative sign. The current
implementation validates every table row, interpolates by absolute feedback RPM, gates
13th and 26th order independently with `order * abs(rpm) / 60 <= 0.45 / dt`, and disables
output when the pump is stopped, speed is outside the calibrated range, phase is invalid,
or the feedback direction reverses.

- [ ] **Step 4: Apply compensation at the RPM boundary and track the base output**

`HYD_MotionControlFB_ScanWithPumpFeedback()` receives the producer packet and a
caller-owned `HYD_RippleCompState` for the current cycle. It passes both by borrowed
pointer through `HYD_ExecuteActiveSegmentControl()`; the legacy `Scan()` supplies null
and retains its exact base-control behavior. Do not retain the packet or state in
`AXIS_REF`, diagnostics, `HYD_MotionControlFB`, or a persistent pressure-controller
object:

```c
base_rpm = pumpOutput->pumpSpeed;
HYD_RippleComp_Scan(feedbackIngress, &defaultTable, rippleState,
                    base_rpm, pumpInput.pumpSpeedLimit, &rippleOutput);
requested_rpm = base_rpm + rippleOutput.deltaRpm;
requested_flow = requested_rpm / pumpInput.flowToPumpSpeedGain;
final_rpm = finalLimiter(requested_flow, requested_rpm);
...
remain converter -> ripple -> final limiter -> PI tracking. A current feedback failure
resets/reseeds the caller-owned state and yields zero compensation; it does not alter the
base PI path. `HYD_PressureController_ExecuteWithPumpFeedback()` keeps the packet as a
borrowed per-scan argument, and freezes only RBF-PI network/Kp/Ki adaptation when the
packet is invalid or incoherent; the positional PI calculation still runs. The default
generated table is an explicitly uncalibrated zero-entry stub, so raw identification
artifacts cannot enable compensation by accident.
...
```bash
git add include/ripple_compensator.h src/ripple_compensator.c include/pressure_ripple_table.h include/motion_control.h src/motion_control.c include/pressure_controller.h src/pressure_controller.c include/rbf_pid.h src/rbf_pid.c tests/test_ripple_compensator.c tests/test_pressure_controller.c tests/test_pump_feedback_chain.c tests/rbf_pid_test.c CMakeLists.txt
...
`sizeof(HYD_RippleCompState)`, `sizeof(RBF_PID_Handle)`, the caller-owned per-axis
state, and confirms `sizeof(HYD_MotionControlFB) <= 3208`. It also runs one million
fixed-step compensator calls and asserts that every output is finite and within the
...
only for a validated candidate pressure recipe. Confirm that the compiler map contains
no ripple state in `HYD_MotionControlFB`, exactly one caller/HAL-adapter ripple state per
enabled axis, no dynamic allocation, and no unresolved feedback symbols. Save the target
build identifier and map summary in
