...
`HYD_TogglePreparedConfig` 内含已提交的原始配置，供实时求解和 IEC 读回共同使用；槽内不得再重复保存第二份 `raw`。
...
按当前字段估计每槽约 `104-112 B`，20 槽约 `2.0-2.2 KiB`。实施后必须通过 `sizeof` 测试给出真实结果，并检查编译器对齐。若超出预算，应先删除重复缓存或非必要包络字段，不能把几何对象重新塞回每个通用轴 FB。
...
几何扫描先覆盖完整物理行程 `[0, Sm]`。所有点都必须满足主曲肘和驱动杆的根式可达条件；任一点真实不可达都拒绝整组配置，不能用 `xGeometryMin` 隐藏几何矛盾。在全行程可达的前提下，允许靠近锁紧区的一段不满足普通速度控制裕量。校验器从 `Sm` 向 `0` 搜索连续安全后缀，并自动推导最小在线位置 `xGeometryMin`：

1. 每个采样点先检查根式可达；随后按归一化雅可比、驱动杆投影、`|k|` 最小值和最大值判定是否适合普通速度控制。
