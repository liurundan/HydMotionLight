...
#define __INIT_GLOBAL_ARRAY(type, name, initial, retained)\
    {\
	    type* value = __GET_GLOBAL_##name();\
        for(int i=0; i< sizeof(value->table)/sizeof(value->table[0]);++i)\
        {\
        	value->table[i] = initial;\
        }\
	    __INIT_RETAIN((*GLOBAL__##name), retained)\
    }
...
// liurundan
#define __INIT_LOCATED_ARRAY(type, location, name, retained)\
{\
    extern type *location;\
	name.value = location;\
    __INIT_RETAIN(name, retained)\
...
/*
#define __INIT_LOCATED_VALUE_ARRAY(name, basetype, initial)\
{\
    basetype initvar = initial;\
    char dsize = sizeof(basetype);\
    char* ptr = (char*)(name.value->table);\
	for(int i=0; i< sizeof(name.value->table)/dsize;++i)\
	{\
		memcpy( (char*)(ptr+i*dsize), &initvar, dsize);\
	}\
}*/

#define __INIT_LOCATED_VALUE_ARRAY(name, type, initial)\
{\
    type temp = initial;\
    *(name.value) = temp;\
}
...
// add liurundan
#define __SET_EXTERNAL_REF(prefix, name, suffix, new_value)\
	if (!(prefix name.flags & __IEC_FORCE_FLAG)) *(*(prefix name.value)) suffix = new_value

#define __SET_LOCATED_ARRAY(prefix, name, suffix, new_value)\
     ( *(prefix name.value) ) suffix = new_value

#define __GET_LOCATED_ARRAY(name, ...)\
     ( (*(name.value)) __VA_ARGS__ )
