...
typedef struct {
  ...
  __DECLARE_VAR(BOOL,EN)
  __DECLARE_VAR(BOOL,ENO)
  __DECLARE_VAR(BOOL,XOUT)

} ALT;

static void ALT_init__(ALT *data__, BOOL retain) {
  __INIT_VAR(data__->EN,__BOOL_LITERAL(TRUE),retain)
  __INIT_VAR(data__->ENO,__BOOL_LITERAL(TRUE),retain)
  __INIT_VAR(data__->XOUT,__BOOL_LITERAL(FALSE),retain)
}

// Code part
static void ALT_body__(ALT *data__) {
  // Control execution
  ...
  __SET_VAR(data__->,XOUT,,!(__GET_VAR(data__->XOUT,)));
...
;  
  ...
  // 1. 处理上升沿 (Start Timer)  
  // 如果处于空闲状态(STATE=0) 且 检测到 IN 的上升沿  
  if ((((__GET_VAR(data__->STATE,) == 0) && !(__GET_VAR(data__->PREV_IN,))) && __GET_VAR(data__->IN,))) {  
    __SET_VAR(data__->,STATE,,1);  
    __SET_VAR(data__->,START_TIME,,__GET_VAR(data__->CURRENT_TIME,));  
    // 注意：这里不再立即置 Q=FALSE，而是让后续逻辑立刻判断时间  
  }  
  
  // 2. 计时逻辑核心 (Execution Logic)  
  // 不再使用 else 分隔上升沿和计时检查，而是顺序执行  
  if (!(__GET_VAR(data__->IN,))) {  
    // IN 为 FALSE：立即复位  
    __SET_VAR(data__->,ET,,__time_to_timespec(1, 0, 0, 0, 0, 0));  
    __SET_VAR(data__->,Q,,__BOOL_LITERAL(FALSE));  
    ...
    // IN 为 TRUE：执行计时检查  
    // 如果 PT=0，这里 (Start + PT) <= Current 立即成立  
    if (LE_TIME(__BOOL_LITERAL(TRUE), NULL, 2, __time_add(__GET_VAR(data__->START_TIME,), __GET_VAR(data__->PT,)), __GET_VAR(data__->CURRENT_TIME,))) {  
      __SET_VAR(data__->,STATE,,2);  
      __SET_VAR(data__->,Q,,__BOOL_LITERAL(TRUE));  
      __SET_VAR(data__->,ET,,__GET_VAR(data__->PT,));  
...
;  
  ...
  // 1. 处理下降沿 (Detect Falling Edge)  
  // 如果处于空闲状态(STATE=0) 且 检测到 IN 的下降沿 (PREV_IN=1, IN=0)  
  if ((((__GET_VAR(data__->STATE,) == 0) && __GET_VAR(data__->PREV_IN,)) && !(__GET_VAR(data__->IN,)))) {  
    __SET_VAR(data__->,STATE,,1);  
    __SET_VAR(data__->,START_TIME,,__GET_VAR(data__->CURRENT_TIME,));  
  }  
  ...
  // 2. 计时逻辑核心 (Execution Logic)  
  // 移除 else，确保下降沿触发后，立马进入下面的逻辑检查时间  
  if (__GET_VAR(data__->IN,)) {  
    // IN 为 TRUE：复位计时器，Q 保持 TRUE (由最后一行逻辑保证)  
    __SET_VAR(data__->,ET,,__time_to_timespec(1, 0, 0, 0, 0, 0));  
    __SET_VAR(data__->,STATE,,0);  
  }   
  else if ((__GET_VAR(data__->STATE,) == 1)) {  
    // IN 为 FALSE 且正在计时 (STATE=1)  
    // 如果 PT=0，这里会立即检测到超时  
    if (LE_TIME(__BOOL_LITERAL(TRUE), NULL, 2, __time_add(__GET_VAR(data__->START_TIME,), __GET_VAR(data__->PT,)), __GET_VAR(data__->CURRENT_TIME,))) {  
      __SET_VAR(data__->,STATE,,2); // 状态变为2（完成）  
      __SET_VAR(data__->,ET,,__GET_VAR(data__->PT,));  
    } else {  
      __SET_VAR(data__->,ET,,__time_sub(__GET_VAR(data__->CURRENT_TIME,), __GET_VAR(data__->START_TIME,)));  
    };  
  ...
  // 3. 计算输出  
  // Q = IN OR (STATE == 1)  
  // 如果 PT=0，上面逻辑已将 STATE 变为 2，且 IN 为 FALSE，所以 Q 立即变为 FALSE  
  __SET_VAR(data__->,Q,,(__GET_VAR(data__->IN,) || (__GET_VAR(data__->STATE,) == 1)));  
