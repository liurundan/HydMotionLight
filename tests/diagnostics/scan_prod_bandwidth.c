...
static Res run(float tau, float omega) {
    ...
    if (tau > 0.0f)   wr(0, HYD_PARAM_PRESSURE_PLANT_TAU, tau);
    if (omega > 0.0f) wr(0, HYD_PARAM_PRESSURE_LOOP_OMEGA, omega);
...
int main(void) {
    const float taus[] = {0.0f, 1.862f, 2.5f};
    const float omegas[] = {8.0f, 12.0f, 16.0f, 20.0f, 24.0f};
    int i, j;
    setvbuf(stdout, NULL, _IONBF, 0);
    printf("%8s %8s | %8s %8s %8s %8s | %s\n", "tau", "omega", "Mp%", "tr_ms", "ess_bar", "sigma", "3/3");
    for (i = 0; i < 3; ++i) {
        for (j = 0; j < 5; ++j) {
