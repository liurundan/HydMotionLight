...
char *PLC_ID = NULL;
volatile  unsigned int PLC_state = 0;
/*
...
int __init_HydMotion(int argc,char **argv);
void __cleanup_HydMotion(void);
void __retrieve_HydMotion(void);
void __publish_HydMotion(void);
...
int __init(int argc,char **argv)
{
    ...
    init_level=1; if((res = __init_HydMotion(argc,argv))){return res;}
...
void __cleanup(void)
{
    if(init_level >= 3) __cleanup_0();
    if(init_level >= 2) __cleanup_HydMotion();
...
/* Variable used to stop plcloop thread  INFINITE */
void PlcLoop()
{
    PLC_shutdown = 0;
    while(!PLC_shutdown) {
        if (WaitForSingleObject(PLC_timer, 5000) != WAIT_OBJECT_0){
            PLC_shutdown = 1;
            break;
        }
...
int stopPLC()
{
    ...
    // wait end of PLC thread INFINITE
    DWORD waitResult = WaitForSingleObject(PLC_thread, 5000);
    switch (waitResult) {
        case WAIT_OBJECT_0:
            // normal termination
            break;
        case WAIT_TIMEOUT:
            ...
            TerminateThread(PLC_thread, 0);  // leak memory, but we are exiting anyway
            break;
        case WAIT_ABANDONED:
			// thread was abandoned, this should not happen
            break;
    }
...
static int PLC_timer_fd = -1;
void PLC_SetTimer(unsigned long long next, unsigned long long period)
{
    struct itimerspec its;
    its.it_value.tv_sec  = next / 1000000000ULL;
    its.it_value.tv_nsec = next % 1000000000ULL;
    if (period < 1000000ULL) period = 1000000ULL;
    its.it_interval.tv_sec  = period / 1000000000ULL;
    its.it_interval.tv_nsec = period % 1000000000ULL;

    if (timerfd_settime(PLC_timer_fd, 0, &its, NULL) < 0) {
...
static sem_t plc_done_sem;

static void* PlcLoop(void* arg)
{
    (void)arg;
    PLC_shutdown = 0;
    while (!PLC_shutdown) {
        ...
        read(PLC_timer_fd, &expirations, sizeof(expirations));

        PLC_GetTime(&__CURRENT_TIME);
        __run();
    }
    sem_post(&plc_done_sem);
    return NULL;
}

static pthread_t PLC_thread;
static sem_t debug_sem;
static sem_t debug_wait_sem;
static sem_t python_sem;
static sem_t python_wait_sem;

#define maxval(a,b) ((a>b)?a:b)
int startPLC(int argc, char **argv)
{
    if (sem_init(&debug_sem, 0, 1) != 0) {
        printf("startPLC sem_init debug_sem error\n");
        return 1;
    }
    if (sem_init(&debug_wait_sem, 0, 0) != 0) {
        printf("startPLC sem_init debug_wait_sem error\n");
        return 1;
    }
    if (sem_init(&python_sem, 0, 1) != 0) {
        printf("startPLC sem_init python_sem error\n");
        return 1;
    }
    if (sem_init(&python_wait_sem, 0, 0) != 0) {
        printf("startPLC sem_init python_wait_sem error\n");
        return 1;
    }
    sem_init(&plc_done_sem, 0, 0);

    PLC_timer_fd = timerfd_create(CLOCK_MONOTONIC, 0);
    if (PLC_timer_fd < 0) {
    ...
    if (__init(argc, argv) == 0) {
        PLC_SetTimer(common_ticktime__, common_ticktime__);
        if (pthread_create(&PLC_thread, NULL, PlcLoop, NULL) != 0) {
...
int stopPLC()
{
    ...
    /* force immediate wakeup of PLC thread */
    struct itimerspec its;
    its.it_value.tv_sec  = 0;
    its.it_value.tv_nsec = 1;
    its.it_interval.tv_sec  = 0;
    its.it_interval.tv_nsec = 0;
    timerfd_settime(PLC_timer_fd, 0, &its, NULL);

    /* wait for PLC thread with 5s timeout */
    struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts);
    ts.tv_sec += 5;
    if (sem_timedwait(&plc_done_sem, &ts) != 0) {
        pthread_cancel(PLC_thread);
        sem_wait(&plc_done_sem);
    }

    __cleanup();

    close(PLC_timer_fd);
    sem_destroy(&debug_wait_sem);
    sem_destroy(&debug_sem);
    sem_destroy(&python_wait_sem);
    sem_destroy(&python_sem);
    sem_destroy(&plc_done_sem);
    return 0;
}

int WaitDebugData(unsigned int *tick)
{
    sem_wait(&debug_wait_sem);
    *tick = __debug_tick;
    return 0;
}
...
int WaitPythonCommands(void)
{
    sem_wait(&python_wait_sem);
    return 0;
}

void UnBlockPythonCommands(void)
{
    sem_post(&python_wait_sem);
}

int TryLockPython(void)
{
    return sem_trywait(&python_sem) == 0;
}
...
void *create_RT_to_nRT_signal(char* name){
    ...
    if (sem_init(&sig->sem, 0, 1) != 0) {
    	char mstr[256];
        snprintf(mstr, 255, "sem_init %s error\n", name);
        LogMessage(LOG_CRITICAL, mstr, strlen(mstr));
        free(sig);
        return NULL;
    }
 ...
 **/
	...
	#ifdef TARGET_TYPE_Gm2xx // liurundan add
	#define LOG_BUFFER_SIZE (1<<10) /* GA-16Ko, GM-10Kb*/
	#else
	#define LOG_BUFFER_SIZE (1<<16) /* GA-16Ko, GM-10Kb*/
	#endif
#endif
