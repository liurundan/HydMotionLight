...
static void test_pressure_handle_maps_percent_to_its_absolute_flow_limit(void) {
    ...
    /* v12：库默认策略已由 PI 改为 RBF_PID。
     * 本用例的"饱和 → HYD_LIMIT_FLAG_FLOW"断言针对的是 **PI 路径**的语义：
     * PI 直接在 final clamp 处削顶，unsaturatedOutputFlow 会超过 maxFlow，
     * 因而置 FLOW 限流标志。RBF_PID 把流量上限内建在 RBF 输出层
     * （output_max_flow / flowRateLimit），unsaturatedOutputFlow 不会超过 maxFlow，
     * 该标志在 RBF 路径下不置位（属已知诊断口径差异，见 docs §18.6）。
     * 为保持本用例原意（压力手柄 25% → 5 L/min 映射 + PI 饱和标志），显式选 PI。 */
    ASSERT_TRUE(HYD_MotionControlFB_WriteParameter(
                    fb, HYD_PARAM_PRESSURE_CONTROLLER_TYPE,
                    (HYD_REAL)HYD_PRESSURE_CONTROLLER_PI),
                "Pressure controller type should be writable");
