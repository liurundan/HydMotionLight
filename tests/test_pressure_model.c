...
static int test_init_params_expose_runtime_defaults(void) {
    ...
    /* Production default is the physical-calibrated plant (2026-09-16): the
     * first-order lag only ever described the pressure-hold case and is now an
     * explicit opt-in used by the legacy regression tests. */
    ASSERT_TRUE(params.model_type == PRESSURE_MODEL_TYPE_PHYSICAL_CALIBRATED);
    ASSERT_TRUE(state.active_model_type == PRESSURE_MODEL_TYPE_PHYSICAL_CALIBRATED);
    /* The opt-in first-order branch keeps the same machine constant as the
     * physical calibration, so the two cannot disagree about Ksys. */
    ASSERT_NEAR(params.first_order_k_bar_per_rpm, 5.0f, 1e-6f);
    ASSERT_NEAR(params.first_order_tau_s, 1.0f, 1e-6f);
    ASSERT_NEAR(params.first_order_delay_s, 0.0f, 1e-6f);
    /* Leakage calibration must stay populated; the openloop gain probe is the
     * end-to-end guard on the resulting Ksys = 5.0 bar/rpm. */
    ASSERT_TRUE(params.physical.outlet_leak_m3_pa_s > 0.0f);
    ASSERT_TRUE(params.physical.pump_leak_c0_m3_pa_s > 0.0f);
    ASSERT_TRUE(params.physical.cylinder_leak_m3_pa_s > 0.0f);
