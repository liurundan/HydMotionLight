...
void test_false_positive_switch_suppression() {
    ...
    HYD_DiagnosticCriteria_CheckFlow(&result, &monitor, &criteria, &state,
                                     0.1, 0.1, false, true);
    assert(!result.triggered);
    assert(result.suppressType == HYD_SUPPRESS_SWITCH);
...
void test_false_positive_loop_build_suppression() {
    ...
    /* 计算闭环建立因子 */
    loopBuildFactor = HYD_CalculateLoopBuildFactor(0.1, 0.2);
    assert(loopBuildFactor == 0.5);

    printf("  ✅ Loop build factor (0.1s / 0.2s) = 0.5\n");

    loopBuildFactor = HYD_CalculateLoopBuildFactor(0.2, 0.2);
    assert(loopBuildFactor == 1.0);

    printf("  ✅ Loop build factor (0.2s / 0.2s) = 1.0\n");

    loopBuildFactor = HYD_CalculateLoopBuildFactor(0.3, 0.2);
    assert(loopBuildFactor == 1.0);
...
void test_severity_escalation_full_flow() {
    ...
    assert(result.action == HYD_PROTECTION_ACTION_WARNING);
    ...
    /* 阶段2：WARNING 持续，即将升级 */
    HYD_DiagnosticCriteria_CheckFaultEscalation(&result, &criteria, &state, 1.5);
    assert(state.warningActive);
    assert(result.severity == HYD_DIAG_SEVERITY_WARNING);
    ...
    /* 阶段3：升级为 FAULT */
    HYD_DiagnosticCriteria_CheckFaultEscalation(&result, &criteria, &state, 2.5);
...
void test_false_positive_reduction_comparison() {
    HYD_ErrorMonitor monitor1, monitor2;
    HYD_DiagnosticCriteria criteria1, criteria2;
    HYD_DiagnosticCriteriaState state1, state2;
    HYD_DiagnosticResult result1, result2;
    int suppressions = 0;
    ...
    /* 模拟100个周期的诊断检查 */
    for (int i = 0; i < 100; i++) {
        HYD_TIME currentTime = (HYD_TIME)i * 0.01;  /* 10ms周期 */
        HYD_TIME segmentElapsedTime = currentTime;

        /* 监视器1：启用误报抑制 */
        if (i == 0) {
            HYD_ErrorMonitor_Init(&monitor1);
            HYD_DiagnosticCriteria_InitState(&state1);
            HYD_DiagnosticCriteria_CreateDefaultPressureCriteria(&criteria1);
        }

        /* 监视器2：禁用误报抑制 */
        if (i == 0) {
            HYD_ErrorMonitor_Init(&monitor2);
            HYD_DiagnosticCriteria_InitState(&state2);
            HYD_DiagnosticCriteria_CreateDefaultPressureCriteria(&criteria2);
        ...
        /* 检查监视器1（启用误报抑制） */
        HYD_DiagnosticCriteria_CheckPressure(&result1, &monitor1, &criteria1, &state1,
                                              currentTime, segmentElapsedTime,
                                              segmentElapsedTime < 0.5, false);

        /* 检查监视器2（禁用误报抑制） */
        HYD_DiagnosticCriteria_CheckPressure(&result2, &monitor2, &criteria2, &state2,
                                              currentTime, segmentElapsedTime,
                                              false, false);

        /* 统计抑制次数 */
        if (result1.suppressType != HYD_SUPPRESS_NONE) {
            suppressions++;
        }
    }
...
void test_end_to_end_injection_molding_scenario() {
    HYD_ErrorMonitor monitor;
    HYD_DiagnosticCriteria pressureCriteria;
    HYD_DiagnosticCriteriaState pressureState;
    HYD_DiagnosticCriteria flowCriteria;
    HYD_DiagnosticCriteriaState flowState;
    HYD_DiagnosticResult pressureResult, flowResult;

    printf("🧪 Testing end-to-end injection molding scenario...\n");

    HYD_ErrorMonitor_Init(&monitor);
    HYD_DiagnosticCriteria_InitState(&pressureState);
    HYD_DiagnosticCriteria_InitState(&flowState);
    HYD_DiagnosticCriteria_CreateDefaultPressureCriteria(&pressureCriteria);
    HYD_DiagnosticCriteria_CreateDefaultFlowCriteria(&flowCriteria);

    /* 模拟注射段 */
    for (int i = 0; i < 50; i++) {
        HYD_TIME currentTime = (HYD_TIME)i * 0.01;
        HYD_TIME segmentElapsedTime = currentTime;
        ...
        /* 检查压力和流量诊断 */
        HYD_DiagnosticCriteria_CheckPressure(&pressureResult, &monitor, &pressureCriteria, &pressureState,
                                              currentTime, segmentElapsedTime,
                                              segmentElapsedTime < 0.5, false);

        HYD_DiagnosticCriteria_CheckFlow(&flowResult, &monitor, &flowCriteria, &flowState,
                                         currentTime, segmentElapsedTime,
                                         false, false);

        /* 启动阶段应该有抑制效果 */
        if (i < 5) {
            if (pressureResult.suppressType == HYD_SUPPRESS_STARTUP) {
                /* 启动抑制生效 */
            }
        }
...
void test_fault_escalation_end_to_end() {
    ...
    HYD_BOOL reachedWarning = false;
    HYD_BOOL reachedFault = false;
    ...
    for (int i = 0; i < 300; i++) {
        HYD_TIME currentTime = (HYD_TIME)i * 0.01;
        HYD_TIME segmentElapsedTime = (HYD_TIME)i * 0.01;
        ...
        if (pressureResult.triggered) {
            ...
            HYD_DiagnosticCriteria_CheckFaultEscalation(&pressureResult, &pressureCriteria, &pressureState, currentTime);

            if (!reachedWarning && pressureResult.severity == HYD_DIAG_SEVERITY_WARNING) {
                reachedWarning = true;
            ...
            if (!reachedFault && pressureState.faultEscalated) {
                ...
                assert(pressureResult.severity == HYD_DIAG_SEVERITY_FAULT);
                assert(pressureResult.action == HYD_PROTECTION_ACTION_STOP);
                break;
            }
        }
    }
...
void test_flow_fault_escalation() {
    ...
    HYD_DiagnosticCriteria_CheckFlow(&flowResult, &monitor, &flowCriteria, &flowState,
                                     0.1, 0.1, false, false);
    assert(flowResult.triggered);
    assert(flowResult.severity == HYD_DIAG_SEVERITY_WARNING);
    HYD_DiagnosticCriteria_CheckFaultEscalation(&flowResult, &flowCriteria, &flowState, 0.1);
    printf("  ✅ Flow WARNING triggered\n");

    /* 持续触发，等待升级 — pass flowResult so escalation can read severity */
    HYD_DiagnosticCriteria_CheckFlow(&flowResult, &monitor, &flowCriteria, &flowState,
                                     2.0, 2.0, false, false);
    HYD_DiagnosticCriteria_CheckFaultEscalation(&flowResult, &flowCriteria, &flowState, 2.0);
    ...
    HYD_DiagnosticCriteria_CheckFlow(&flowResult, &monitor, &flowCriteria, &flowState,
                                     4.0, 4.0, false, false);
    HYD_DiagnosticCriteria_CheckFaultEscalation(&flowResult, &flowCriteria, &flowState, 4.0);
    assert(flowState.faultEscalated);
    assert(flowResult.severity == HYD_DIAG_SEVERITY_FAULT);
    assert(flowResult.action == HYD_PROTECTION_ACTION_STOP);
